...
public interface ScheduleRepository extends JpaRepository<Schedule, Long> {
    @Query("SELECT new com.sparta.schedulemanagement2.dto.ResponseDto(s.title,s.contents,"+"(SELECT COUNT(c) FROM Comment c WHERE c.schedule.id = s.id),s.date,s.fixdate,s.userid)"+"FROM Schedule s")
